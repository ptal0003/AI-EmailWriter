...
function showMessage(){
    ...
    newDiv.addEventListener("click", function(){
        ...
        let mailTextId = newBtn.id.split("_")[0];
        window.alert(document.getElementById(mailTextId).innerText)
...
document.addEventListener("click", function(){
    ...
    for(let i = 0; i < actionButtonSections.length; i++){
        ...
        if(actionButtonSections[i].getElementsByClassName("btC")[0]){
            if(!actionButtonSections[i].getElementsByClassName("btC")[0].getElementsByClassName("email-ai-writer-btn")[0]){
                newBtn.id = actionButtonSections[i].getElementsByClassName("btC")[0].closest(".G3.G2").getElementsByClassName("a3s aiL")[0].id + "_btn"
                newDiv.appendChild(newBtn)
                actionButtonSections[i].getElementsByClassName("btC")[0].appendChild(newDiv)
                replyBoxActionBarReferences.push(actionButtonSections[i].getElementsByClassName("btC")[0]);
                mailBoxReferences.push(actionButtonSections[i].getElementsByClassName("btC")[0].closest(".G3.G2").getElementsByClassName("a3s aiL")[0]);
            }
...
const showModal = (mailTextId) => {
    ...
    <div>
        ...
        <textarea type="text" id="email_context" style="min-height: 100px; width: 450px; font-size: 13px; resize: vertical" ></textarea>
        <h4>
        How would you like to reply to the email?
        </h4>
        <input placeholder = "Enter a brief reply to the email:" style = "width: 450px; font-size: 13px;"></input>
    </div>
